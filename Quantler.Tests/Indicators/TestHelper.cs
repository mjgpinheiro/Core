    ...
    public static class TestHelper
    {
        ...
        public static Action<IndicatorBase<IndicatorDataPoint>, double> AssertDeltaDecreases(double epsilon)
        {
            ...
            return (indicator, expected) =>
            {
                ...
                var currentDelta = Math.Abs((double)indicator.Current.Price - expected);
                if (currentDelta - delta > epsilon)
                {
                    throw new Exception("The delta increased!");
        ...
        public static void AssertIndicatorIsInDefaultState<T>(IndicatorBase<T> indicator)
            where T : DataPoint
        {
            Assert.Equal(0m, indicator.Current.Price);
            Assert.Equal(DateTime.MinValue, indicator.Current.Occured);
            Assert.Equal(0, indicator.Samples);
        ...
        public static IEnumerable<IndicatorDataPoint> GetDataStream(int count, Func<int, decimal> valueProducer = null)
        {
            ...
            {
                yield return new IndicatorDataPoint(reference.AddSeconds(i), TimeZone.Utc, valueProducer.Invoke(i));
            }
        }
        ...
        public static IEnumerable<TradeBar> GetTradeBarStream(string externalDataFilename, bool fileHasVolume = true)
        {
            ...
            {
                Occured = Time.TryParseDate(values.GetCsvValue("date", "time"), out DateTime returnvalue) ? returnvalue : throw new Exception($"Could not parse date {values.GetCsvValue("date", "time")}"),
        ...
        {
            TestIndicator(indicator, "spy_with_indicators.txt", targetColumn,
                (i, expected) => ((double) i.Current.Price).Should().BeApproximately(expected, epsilon));
        }
        ...
        public static void TestIndicator(IndicatorBase<IndicatorDataPoint> indicator, string externalDataFilename, string targetColumn, Action<IndicatorBase<IndicatorDataPoint>, double> customAssertion)
        {
            ...
            foreach (var line in File.ReadLines(Path.Combine("TestData", externalDataFilename)))
            {
                ...
                if (first)
                {
                    ...
                    {
                        throw new Exception($"Didn't find one of 'Close' or '{targetColumn}' in the header: " + line);
                    }
                ...
                DateTime date = Time.TryParseDate(parts[0], out DateTime returnvalue)
                    ? returnvalue
                    : throw new Exception($"Could not parse date {parts[0]}");

                var data = new IndicatorDataPoint(date, TimeZone.Utc, close);
        ...
        /// <param name="targetColumn">The column with the correct answers</param>
        /// <param name="epsilon">The maximum delta between expected and actual</param>
        public static void TestIndicator(IndicatorBase<DataPointBar> indicator, string externalDataFilename, string targetColumn, double epsilon = 1e-3) =>
            TestIndicator(indicator, externalDataFilename, targetColumn,
                (i, expected) => ((double) i.Current.Price).Should()
                    .BeApproximately(expected, epsilon, "Failed at " + i.Current.Occured.ToString("o")));
        ...
        public static void TestIndicator(IndicatorBase<TradeBar> indicator, string externalDataFilename, string targetColumn, double epsilon = 1e-3) =>
            TestIndicator(indicator, externalDataFilename, targetColumn, (i, expected) => ((double)i.Current.Price).Should()
                .BeApproximately(expected, epsilon, "Failed at " + i.Current.Occured.ToString("o")));
        ...
        /// <param name="targetColumn">The column with the correct answers</param>
        /// <param name="customAssertion">Sets custom assertion logic, parameter is the indicator, expected value from the file</param>
        public static void TestIndicator(IndicatorBase<DataPointBar> indicator, string externalDataFilename, string targetColumn, Action<IndicatorBase<DataPointBar>, double> customAssertion)
        {
            ...
            foreach (var line in File.ReadLines(Path.Combine("TestData", externalDataFilename)))
            {
                ...
                {
                    Occured = Time.TryParseDate(parts[0], out DateTime returnvalue)
                        ? returnvalue
                        : throw new Exception($"Could not parse date {parts[0]}"),
        ...
        /// <param name="indicator">The indicator under test</param>
        /// <param name="externalDataFilename">The external CSV file name</param>
        public static void TestIndicatorReset(IndicatorBase<DataPointBar> indicator, string externalDataFilename)
        {
        ...
        public static void TestIndicatorReset(IndicatorBase<IndicatorDataPoint> indicator, string externalDataFilename)
        {
            ...
            foreach (var data in GetTradeBarStream(externalDataFilename, false))
            {
                indicator.Update(date, TimeZone.Utc, data.Close);
            }
