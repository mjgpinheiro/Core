...
namespace Quantler.Tests.Indicators
{
    ...
    public class LeastSquaresMovingAverageTest
    {
        ...
        #endregion Array expected

        #region Public Methods

        [Fact]
        [Trait("Quantler.Indicators", "LeastSquaresMovingAverage")]
        public void LSMAComputesCorrectly()
        {
            ...
            {
                LSMA.Update(new IndicatorDataPoint(time, TimeZone.Utc, prices[i]));
                decimal LSMAValue = Math.Round(LSMA.Current.Price, 4);
                actual[i] = LSMAValue;
        ...
        public void ResetsProperly()
        {
            ...
            {
                LSMA.Update(new IndicatorDataPoint(time, TimeZone.Utc, 1m));
                time = time.AddMinutes(1);
            }
            Assert.True(LSMA.IsReady, "LSMA ready");
            LSMA.Reset();
