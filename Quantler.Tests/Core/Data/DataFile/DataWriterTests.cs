...
namespace Quantler.Tests.Core.Data.DataFile
{
    ...
    {
        //Data input for unit tests
        private readonly List<DataPoint> _inputDataPoints = new List<DataPoint>
        {
        ...
        [Fact(Skip = "ToDo")]
        [Trait("Quantler.Core.Data.DataFile", "DataWriter")]
        public void CanWriteCorrectData()
        {
            //Arrange
            var sut = new DataWriter(_path, "TST", 20170101, "na", DataType.Unknown);

            //Act
            _inputDataPoints.ForEach(x => sut.Feed(x));

            //Assert
            sut.Count.Should().Be(_inputDataPoints.Count);
        }

        [Fact(Skip = "ToDo")]
        [Trait("Quantler.Core.Data.DataFile", "DataWriter")]
        public void HasCorrectFileName()
        {
            //Arrange
            var sut = new DataWriter(_path, "TST", 20170101, "na", DataType.Unknown);
            ...
            //Assert
            var fileInfo = new FileInfo(sut.Filepath);
            fileInfo.Extension.Should().Be("DAT");
            fileInfo.Name.Should().Be("");
            fileInfo.DirectoryName.Should().Be("");
            fileInfo.Directory.Parent.Name.Should().Be("");
        }
    }
}
