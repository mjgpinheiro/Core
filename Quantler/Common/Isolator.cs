        ...
        public bool ExecuteWithTimeLimit(TimeSpan timeSpan, Func<string> withinCustomLimits, Action codeBlock, long memoryCap = 1024)
        {
            ...
            while (!task.IsCompleted && DateTime.Now < end)
            {
                ...
                if (DateTime.Now > memoryLogger)
                {
                    ...
                    {
                        _log.Error("Execution Security Error: Memory usage over 80% capacity. Sampled at {0}", sample);
                    }
                    _log.Trace("{0} Isolator.ExecuteWithTimeLimit(): Used: {1} Sample: {2}", DateTime.Now.ToString("u"), PrettyFormatRam(memoryUsed), PrettyFormatRam((long)sample));
            ...
            if (task.IsCompleted == false && message == "")
            {
                ...
                _log.Trace("Isolator.ExecuteWithTimeLimit(): " + message);
            }

            if (message != "")
            {
                CancellationTokenSource.Cancel();
                _log.Error("Security.ExecuteWithTimeLimit(): " + message);
