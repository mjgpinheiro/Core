...
namespace Quantler.Indicators
{
        ...
        where T : DataPoint
    {
        ...
        {
            Name = name;
            Current = new IndicatorDataPoint(DateTime.MinValue, TimeZone.Utc, 0m);
        }
        ...
        public override bool Equals(object obj)
        {
            ...
            return Current.Price == converted;
        }
        ...
        public virtual void Reset()
        {
            ...
            Current = new IndicatorDataPoint(DateTime.MinValue, TimeZone.Utc, default(decimal));
        }
        ...
        {
            if (_previousInput != null && input.Occured < _previousInput.Occured)
            {
                // if we receive a time in the past, throw
                throw new ArgumentException(string.Format("This is a forward only indicator: {0} Input: {1} Previous: {2}", Name, input.Occured.ToString("u"), _previousInput.Occured.ToString("u")));
            }
            if (!ReferenceEquals(input, _previousInput))
            {
                ...
                {
                    Current = new IndicatorDataPoint(input.Occured, input.TimeZone, nextResult.Value);
