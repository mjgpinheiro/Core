...
namespace Quantler.Indicators
{
    ...
    public static class IndicatorExtensions
    {
        ...
        /// <param name="timezone">Input timezone</param>
        /// <param name="value">The value to use to update this indicator</param>
        /// <returns>True if this indicator is ready, false otherwise</returns>
        public static bool Update(this IndicatorBase<IndicatorDataPoint> indicator, DateTime time, TimeZone timezone, decimal value)
        {
            return indicator.Update(new IndicatorDataPoint(time, timezone, value));
        }
            ...
            where TWeight : IndicatorBase<IndicatorDataPoint>
        {
            ...
            value.Updated += (sender, aggregated) =>
            {
                x.Update(aggregated);
                if (x.Samples == y.Samples)
                {
                    numerator.Update(aggregated.Occured, aggregated.TimeZone, aggregated.Price * y.Current.Price);
                }
            };

            weight.Updated += (sender, aggregated) =>
            {
                y.Update(aggregated);
                if (x.Samples == y.Samples)
                {
                    numerator.Update(aggregated.Occured, aggregated.TimeZone, aggregated.Price * x.Current.Price);
                }
                denominator.Update(aggregated);
            };
