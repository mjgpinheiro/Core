...
namespace Quantler.Indicators
{
    ...
    public class VolumeWeightedAveragePriceIndicator : TradeBarIndicator
    {
        ...
        {
            _price.Update(input.EndTime, input.TimeZone, (input.Open + input.High + input.Low + input.Price) / 4);
            _volume.Update(input.EndTime, input.TimeZone, input.Volume);
            return _vwap.Current.Price;
        }
