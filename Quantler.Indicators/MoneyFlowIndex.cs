...
namespace Quantler.Indicators
{
    ...
    public class MoneyFlowIndex : TradeBarIndicator
    {
        ...
        /// <summary>The current and previous typical price is used to determine positive or negative money flow</summary>
        ...
        protected override decimal ComputeNextValue(TradeBar input)
        {
            ...
            PositiveMoneyFlow.Update(input.Occured, input.TimeZone, typicalPrice > PreviousTypicalPrice ? moneyFlow : 0.0m);
            NegativeMoneyFlow.Update(input.Occured, input.TimeZone, typicalPrice < PreviousTypicalPrice ? moneyFlow : 0.0m);
            PreviousTypicalPrice = typicalPrice;

            decimal totalMoneyFlow = PositiveMoneyFlow.Current.Price + NegativeMoneyFlow.Current.Price;
            if (totalMoneyFlow == 0.0m)
            {
                return 100.0m;
            }

            return 100m * PositiveMoneyFlow.Current.Price / totalMoneyFlow;
        }
